...
ORG_NAME = "pulp"
REPO_NAME = "pulp_ansible"
APP_NAME = "pulp_ansible"
PRIMARY_BRANCH = "main"


def get_current_branch():
    pid = subprocess.run(
        "git branch --show-current", shell=True, stdout=subprocess.PIPE, check=True
    )
...
def get_changed_files(pr_branch, target_branch=PRIMARY_BRANCH):
    if os.environ.get("GITHUB_PR_NUMBER"):
        url = f"https://patch-diff.githubusercontent.com/raw/{ORG_NAME}/{REPO_NAME}"
...
def verify_test_files_changed(changed_files):
    ...
    app_paths = [f"{APP_NAME}/"]

    # places where test code goes
    test_paths = [f"{APP_NAME}/tests"]
...
def main():
    ...
    skip_users = ["dependabot[bot]", "patchback[bot]", "pulpbot", "pulpbot[bot]"]
    ...
    target_branch = os.environ.get("GITHUB_BASE_REF", PRIMARY_BRANCH)
    print(f"TARGET-BRANCH: {target_branch}")
    if target_branch != PRIMARY_BRANCH:
        print(f"TDD is only enforced on the {PRIMARY_BRANCH} branch")
        sys.exit(0)
