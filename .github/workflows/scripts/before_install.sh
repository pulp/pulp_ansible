...
COMPONENT_SOURCE="./pulp_ansible/dist/pulp_ansible-${COMPONENT_VERSION}-py3-none-any.whl"
if [ "$TEST" = "s3" ]; then
...
legacy_component_name: "pulp_ansible"
component_name: "ansible"
component_version: "${COMPONENT_VERSION}"
pulp_env: {}
pulp_settings: {"allowed_export_paths": "/tmp", "allowed_import_paths": "/tmp", "ansible_api_hostname": "https://pulp:443", "ansible_content_hostname": "https://pulp:443/pulp/content", "ansible_signature_require_verification": false, "api_root": "/pulp/"}
pulp_scheme: "https"
image:
  name: "pulp"
  tag: "ci_build"
  ...
  webserver_snippet: $(test -f pulp_ansible/app/webserver_snippets/nginx.conf && echo -n true || echo -n false )
extra_files:
  - origin: "pulp_ansible"
    destination: "pulp_ansible"
services:
  - name: "pulp"
    image: "pulp:ci_build"
    volumes:
...
if [ "$TEST" = "gcp" ]; then
  ...
  - name: "ci-gcp"
    ...
    command: " -scheme http"
gcp_test: true
pulp_scenario_settings: {"MEDIA_ROOT": "", "STORAGES": {"default": {"BACKEND": "storages.backends.gcloud.GoogleCloudStorage", "OPTIONS": {"bucket_name": "gcppulp", "custom_endpoint": "http://ci-gcp:4443"}}, "staticfiles": {"BACKEND": "django.contrib.staticfiles.storage.StaticFilesStorage"}}, "ansible_collect_download_count": false, "ansible_collect_download_log": true}
pulp_scenario_env: {}
VARSYAML
fi
