...
export response=$(curl --write-out %{http_code} --silent --output /dev/null https://pypi.org/project/pulp-ansible/$1/)
if [ "$response" == "200" ];
then
  echo "pulp_ansible $1 has already been released. Skipping."
  exit
fi
