...
class AnsibleDeclarativeVersion(DeclarativeVersion):
    ...
    def pipeline_stages(self, new_version):
        ...
        """
        repository = new_version.repository.cast()
        ...
        pipeline = [
            ...
            QueryExistingContents(),
            DocsBlobDownloader(),
            CollectionContentSaver(repository),
            RemoteArtifactSaver(),
            ResolveContentFutures(),
        ]
    ...
    async def _pre_save(self, batch):
        ...
        for d_content in batch:
            ...
            if d_content.extra_data.get("deprecated", False):
                to_deprecate.append(
                    AnsibleCollectionDeprecated(repository=self.repository, collection=collection)
                )
            else:
                to_undeprecate.append(collection)
        ...
        if to_undeprecate:
            AnsibleCollectionDeprecated.objects.filter(
                repository=self.repository, collection__in=to_undeprecate
            ).delete()
