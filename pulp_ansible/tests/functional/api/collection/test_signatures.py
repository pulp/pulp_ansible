...
def test_upload_signature_with_key_type(
    ...
    certificate = tsk.extract_certificate()
    public_key = str(certificate)
    ...
    with open(signature_file, "wb") as f:
        f.write(sig)
    signer_fingerprint = pysequoia.Sig.from_bytes(sig).issuer_fingerprint.upper()
    ...
    signature = ansible_bindings.ContentCollectionSignaturesApi.read(signature_href)
    assert signature.pubkey_fingerprint == signer_fingerprint
